...
function custom_logo_class( $logo ) {
    ...
    $logo = preg_replace( "/(<a.+?)class=(['\"])(.+?)(['\"])/i", '$1class=$2app-header__logo-link $3$4', $logo, 1 );

    return preg_replace( "/(<img.+?)class=(['\"])(.+?)(['\"])/i", '$1class=$2app-header__logo $3$4', $logo, 1 );
}
...
function comment_form_default_fields( $fields ) {
    ...
    array_walk( $fields, static function ( &$field, $key ) {
...
function widget_class_filter( $params ) {
    ...
    if ( ! empty( $matches ) && ! array_diff_key( array_flip( [ 1, 2, 3, 4 ] ), $matches ) ) {
        ...
        $params[0]['before_widget'] = str_replace(
            $matches[1],
            $matches[2] . implode( ' ', $_classes ) . $matches[4],
    ...
    if ( is_home() ) {
        ...
        if ( current_theme_supports( 'post-formats' ) && post_type_supports( $post_type, 'post-formats' ) ) {
            ...
            $classes[] = $post_format && ! is_wp_error( $post_format )
                ? "{$post_type}-format-{$post_format}"
                : "{$post_type}-format-standard";
        }
